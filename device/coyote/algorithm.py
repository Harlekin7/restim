...
- Uses adaptive packet scheduling (80% of packet duration for seamless transitions)
...
class CoyoteAlgorithm:
    """Base class for Coyote algorithms. Subclasses implement _positional_intensity()."""
...
class CoyoteThreePhaseAlgorithm(CoyoteAlgorithm):
    ...
    def _positional_intensity(self, time_s: float, volume: float) -> Tuple[int, int]:
        ...
        intensity_a = float(p ** exponent)
        intensity_b = float((1 - p) ** exponent)

        # Apply balance calibration between channel A and B
        balance = float(self.params.calibrate.neutral.last_value())
        intensity_a *= min(1.0, 10**(balance/10))
        intensity_b *= min(1.0, 10**(-balance/10))
...
class CoyoteTwoChannelAlgorithm(CoyoteAlgorithm):
    ...
    def _positional_intensity(self, time_s: float, volume: float) -> Tuple[int, int]:
        ...
        w_left = float(max(0.0, (beta + 1.0) / 2.0))
        w_right = float(max(0.0, (1.0 - beta) / 2.0))
        w_neutral = float(max(0.0, alpha))
        ...
        scale = float(ThreePhaseCenterCalibration(center_db).get_scale(alpha, beta))
